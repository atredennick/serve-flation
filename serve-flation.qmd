...
formula <- serve_speed ~ server * year + slam  # Predict serve speed with server, year, and slam, plus interaction

# Fit the Weibull distribution model with horseshoe priors
fit_weibull_horseshoe <- brm(
  formula = formula,           # Model serve_speed as a function of server, year, and slam
  family = weibull(),          # Use the Weibull distribution
  data = your_data,            # Your dataset containing serve speeds
  prior = c(
    prior(horseshoe(1), class = "b"),   # Horseshoe prior for regression coefficients
    prior(lognormal(0, 1), class = "Intercept"),  # Prior for the intercept term
    prior(lognormal(0, 1), class = "sd")   # Prior for the scale parameter (optional)
  ),
